...
# Usage:
# ./2_run-transfer-plugin-install.sh <JFROG_HELM_RELEASE_NAME> <JFROG_PLATFORM_NAMESPACE> <FULL_PATH_TO_SCRIPT> [JFROG_ACCESS_TOKEN]
#
# Example:
...
# bash ./install_source_rt_plugins/1_run-transfer-plugin-install.sh jfrog1-release ps-jfrog-platform ./install_source_rt_plugins/2_install-transfer-plugin.sh abc123mytoken
# OR
# export JFROG_ACCESS_TOKEN=abc123mytoken
# bash ./install_source_rt_plugins/1_run-transfer-plugin-install.sh jfrog1-release ps-jfrog-platform ./install_source_rt_plugins/2_install-transfer-plugin.sh
...
# Get all Artifactory pods for this release
# echo "🔍 Finding all Artifactory pods for release ${HELM_RELEASE}..."
# PODS=$(kubectl get pods -n "${NAMESPACE}" -o jsonpath='{.items[*].metadata.name}' | tr ' ' '\n' | grep "^${HELM_RELEASE}-artifactory-[0-9]\+$")
...
echo "🚀 Executing ${SCRIPT_NAME} with token and Artifactory URL inside '${CONTAINER_NAME}' of pod ${FIRST_RUNNING_ARTIFACTORY_POD}..."
