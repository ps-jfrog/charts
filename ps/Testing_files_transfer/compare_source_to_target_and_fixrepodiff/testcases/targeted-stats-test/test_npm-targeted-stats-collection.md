...
- [README.md](README.md) — Docker-based delta sync test: initial full sync, publish a delta of new images, crawl the delta, and run targeted stats with `--skip-pass1`. Includes both test (separate directories) and production (single directory) approaches, plus troubleshooting for older `comparison.db` missing `targeted_uris`.
