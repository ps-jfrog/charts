...
# ----------------------------------------------------------------------------
COMMAND_AUDIT_FILE="compare-and-reconcile-command-audit-$(date +%Y%m%d-%H%M%S).log"
echo "Command audit log: $COMMAND_AUDIT_FILE"

_audit_run() {
  local _c="$*"
  echo "$_c"
  echo "$_c" >> "$COMMAND_AUDIT_FILE"
  eval "$_c"
}
...
if [ "$COMPARE_SOURCE_NEXUS" == "1" ]; then
    ...
    _audit_run $COMMAND_NAME authority-add "$SOURCE_NEXUS_AUTHORITY" "$SOURCE_NEXUS_BASE_URL"
    if [ -n "${NEXUS_ADMIN_TOKEN:-}" ]; then
        _audit_run $COMMAND_NAME credentials-add "$SOURCE_NEXUS_AUTHORITY" --bearer-token="$NEXUS_ADMIN_TOKEN" --discovery=nexus_assets
    elif [ -n "${NEXUS_ADMIN_USERNAME:-}" ] && [ -n "${NEXUS_ADMIN_PASSWORD:-}" ]; then
        _audit_run $COMMAND_NAME credentials-add "$SOURCE_NEXUS_AUTHORITY" --username="$NEXUS_ADMIN_USERNAME" --password="$NEXUS_ADMIN_PASSWORD" --discovery=nexus_assets
    else
    ...
    if [ -f "$NEXUS_REPOSITORIES_FILE" ]; then
        ...
        while IFS= read -r repo; do
            ...
            _audit_run $COMMAND_NAME list "$SOURCE_NEXUS_AUTHORITY" --repository="$repo" --run-id="$NEXUS_RUN_ID"
        done < "$NEXUS_REPOSITORIES_FILE"
    else
        _audit_run $COMMAND_NAME list "$SOURCE_NEXUS_AUTHORITY"
    fi
    echo ""
fi
...
if [ "$RECONCILE" == "1" ]; then
    ...
    for entry in "${RECONCILE_VIEWS[@]}"; do
        ...
        tmpf=$(mktemp)
        _cmd="$COMMAND_NAME report --jsonl $view"
        echo "$_cmd"
        echo "$_cmd" >> "$COMMAND_AUDIT_FILE"
